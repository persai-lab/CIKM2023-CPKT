...
class LLPKTFast(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.cuda:
            ...
            prBlue("Torch Random Number: {} ".format(torch.randint(0, 100, (3,))))
            prBlue("Numpy Random Number: {} ".format(np.random.randint(0, 100, (3,))))
            prBlue("Random Number: {} ".format(random.randint(0, 100)))
        else:
            random.seed(config.seed)
            np.random.seed(config.seed)
            torch.manual_seed(config.seed)
        ...
        print(self.key_matrix_embed.weight)
    ...
    def forward(self, q_data, qa_data, users, mode, update_memory=False):
        ...
        for i in range(seq_len):
            ...
            if mode == "online" and update_memory and i == self.test_size:
